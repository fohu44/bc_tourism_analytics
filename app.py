...
# --- Advanced Analytics with Fully Dynamic Annotations ---
@app.callback(
...
def render_advanced(chosen, mode, y1, y2, y3, start, end, metrics):
    ...
    # 2) Stacked Area Chart (unchanged)
    ...
    # 3) Event Annotations – now always filtered
    if 'annotations' in chosen:
        # derive a single window for both modes
        if mode == 'year':
            yrs = [y1] + ([y2] if y2 else []) + ([y3] if y3 else [])
            start_dt = pd.to_datetime(f"{min(yrs)}-01-01")
            end_dt   = pd.to_datetime(f"{max(yrs)}-12-31")
        else:
            start_dt = pd.to_datetime(start)
            end_dt   = pd.to_datetime(end)

        # filter events to only those overlapping window
        ev = df_events.copy()
        ev['start'] = pd.to_datetime(ev['start'])
        ev['end']   = pd.to_datetime(ev['end'])
        ev = ev[(ev['start'] <= end_dt) & (ev['end'] >= start_dt)]
        ...
        if ev.empty:
            ...
            fig3 = px.timeline(
                ...
                color='label', color_discrete_map={e['label']:e['color'] for e in events}
            )
