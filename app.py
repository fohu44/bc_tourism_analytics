...
events = [
    {'label': 'COVID-19',      'start': '2020-03-01', 'end': '2022-06-01', 'color': 'lightcoral'},
    {'label': 'Wildfires 2021', 'start': '2021-07-01', 'end': '2021-09-01', 'color': 'lightgray'}
]
...
date_options = [
    {'label': dt.strftime('%b %Y'), 'value': dt.strftime('%Y-%m-%d')}
    for dt in df.index
]
...
def update_core(mode, y1, y2, y3, start, end, metrics):
    ...
    if mode=='year':
        ...
        # Line chart
        line_fig=px.line(
            long_df,x='Month',y='Value',
            color='Year',line_dash='Metric',
            markers=True,title="Year-to-Year Comparison"
        )
        line_fig.update_layout(template='plotly_white')

        # Bar chart: color by Metric + pattern by Year → two legends
        bar_fig=px.bar(
            long_df,
            x='Month',y='Value',
            color='Metric',
...
def render_advanced(chosen, metrics):
    ...
    if 'horizon' in chosen and metrics:
        ...
        fig=go.Figure(); colors=['#d0e1f9','#74a9cf','#0570b0']
        for i,th in enumerate(levels):
            band=s.clip(upper=th)
            if i>0: band-=levels[i-1]
            fig.add_bar(x=s.index,y=band,marker_color=colors[i],name=f"Band {i+1}")
        fig.update_layout(
            barmode='stack',
            title=f"Horizon Chart: {all_metric_labels[m]}",
            legend_title_text="Intensity Bands",
            xaxis_title="Date",yaxis=dict(visible=False),
            template='plotly_white'
        )
        horizon=dcc.Graph(figure=fig)

    # Stacked area
    if 'stacked_area' in chosen:
        melt=df.reset_index().melt(
            id_vars=['period'],
            value_vars=['asia','europe','other'],
            var_name='Market',value_name='Visitors'
        )
        melt['Market']=melt['Market'].map({'asia':'Asia','europe':'Europe','other':'Other'})
        fig2=px.area(
            melt,x='period',y='Visitors',
            color='Market',
            title="Visitor Origin Composition Over Time",
            labels={'period':'Date'}
        )
    ...
    if 'annotations' in chosen:
        fig3=px.timeline(
            df_events,x_start='start',x_end='end',y='label',
            color='label',
